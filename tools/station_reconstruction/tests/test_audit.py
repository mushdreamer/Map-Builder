...
class AuditTests(unittest.TestCase):
    ...
    def test_unity_manifest_contains_only_strict_tier_a_placements(self):
        manifest = {
            "tmx": [{"tileWidth": 10}], "psd": [{"width": 20, "height": 20}],
            "png": [{"path": "asset.png", "assetKey": "asset", "assetRole": "base",
                     "noCollision": False, "footprintHint": {"cols": 1, "rows": 1}}],
            "matches": [{"assetPath": "asset.png", "candidates": [{
                "layerPath": "confirmed", "layerBounds": [0, 0, 10, 10],
                "zIndex": 0, "transform": "identity", "confidence": 0.97,
                "scoreMargin": 0.04,
            }]}],
        }
        placement = audit.build_unity_placements(manifest)["placements"][0]
        self.assertEqual("autoAccepted", placement["reviewState"])
        self.assertEqual("A", placement["tier"])
