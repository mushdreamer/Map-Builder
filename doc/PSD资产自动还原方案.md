...
**重要限制：当前仓库并不包含需求中所述的 TMX、PSD、最终概念图和约 100 个交付 PNG。**仓库里只有 Kenney 示例素材（例如 `Sample.png` 与 `PNG/Tiles/tile_*.png`），因此本轮可以完整审阅现有 Unity 数据结构和坐标/导出链路，并给出实施设计，但不能对真实项目数据验证 PSD 图层可用率、命名规律、对齐误差或最终还原率。正式开发前应先做“数据体检”（见第 7 节），不可直接大改 Unity 代码。
